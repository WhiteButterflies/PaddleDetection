...
class ImageIOCapture:
    def __init__(self, device_name):
        import imageio
        self.reader = imageio.get_reader(device_name)
        self._iterator = iter(self.reader)
        self.meta = self.reader.get_meta_data()
    def read(self):
        try:
            frame_rgb = next(self._iterator)  # RGB 格式
    ...
    def get(self, prop_id):
        if prop_id == cv2.CAP_PROP_FRAME_WIDTH:
            ...
            raw_frame_count= self.meta.get("nframes", None)
            if raw_frame_count is None or math.isinf(raw_frame_count):
                frame_count = -1
            else:
    ...
    for key in cfg.keys():
        if type(cfg[key]) ==  dict and \
            ...
            if "model_dir" in cfg[key].keys():
                ...
                downloaded_rec_model_dir = auto_download_model(rec_mmp4odel_dir)
...
class PipePredictor(object):
        ...
        if not is_video:
            ...
            if self.with_mot or self.modebase["idbased"] or self.modebase[
                ...
                self.mot_predictor = SDE_Detector(
                    ...
                    args.cpu_threads,
                    args.enable_mkldnn,
                    threshold = 0.01,#added by liu for Detector threshold ori 0.5
    ...
    def v_shape_sort_update_and_reorder_full(self,res):
        for frame_index, frame_data in enumerate(res):
            xywh_list = frame_data[0][0]
            scores = frame_data[1][0]
            ids = frame_data[2][0]
            ...
            boxes = [
                {
                    ...
                    "id": id_,
                    "left_x": xywh[0],
                    "bottom_y": xywh[1] + xywh[3],  # y + h
                }
                for i, (xywh, score, id_) in enumerate(zip(xywh_list, scores, ids))
            ]
            ...
            # Step 1: 左下角 x 最小
            temp_boxes.sort(key=lambda b: b["left_x"])
            A = temp_boxes.pop(0)

            # Step 2: 左下角 y 最大
            temp_boxes.sort(key=lambda b: b["bottom_y"], reverse=True)
            B = temp_boxes.pop(0)
            ...
            # 分配 ID = 1, 2, 3
            id_map = {box["index"]: new_id for new_id, box in enumerate(v_boxes, start=1)}

            # 构建新的 ID 列表（保留其余框的原始 ID）
            new_ids = []
            for i in range(len(ids)):
                new_ids.append(id_map[i] if i in id_map else ids[i])
            frame_data[2][0] = new_ids
            ...
            updated_boxes = [
                ...
                for xywh, score, new_id in zip(xywh_list, scores, new_ids)
            ]

            # 最终按 ID 升序排序
            updated_boxes.sort(key=lambda b: b["id"])

            # 更新回 frame
            frame_data[0][0] = [b["xywh"] for b in updated_boxes]
            frame_data[1][0] = [b["score"] for b in updated_boxes]
            frame_data[2][0] = [b["id"] for b in updated_boxes]

    #added by liu for detect rgb
    def detect_red_upper_body(self,image, threshold=0.2):

        # 截取上半部分
        # cv2.imshow('image', image)
        height, width = image.shape[:2]
        # upper_half = image[height // 8:height // 2, width//4:width-width//4]
        # upper_half = image[height // 8:height // 2, width//4:width-width//4]
        height_thre=int(height*0.4)
        height_thre2=int(height*0.1)
        width_thre=int(width*0.2)
        upper_half = image[height//2-height_thre:height // 2+height_thre2, width // 2-width_thre: width // 2+width_thre]
        # cv2.imshow('upper_half', cv2.cvtColor(upper_half, cv2.COLOR_RGB2BGR))
        # 转换到HSV
        ...
        if  red_ratio > threshold:
            cv2.imshow('Target', cv2.cvtColor(upper_half,cv2.COLOR_RGB2BGR))
            cv2.moveWindow('Target',20,20)

        return red_ratio > threshold, red_ratio

    def detect_white_upper_body(self,image, threshold=0.2):
        height, width = image.shape[:2]
        height_thre = int(height * 0.4)
        height_thre2 = int(height * 0.1)
        width_thre = int(width * 0.2)
        upper_half = image[height // 2 - height_thre: height // 2 + height_thre2,
                     width // 2 - width_thre: width // 2 + width_thre]
        ...
        if white_ratio > threshold:
            cv2.imshow('White Target', cv2.cvtColor(upper_half, cv2.COLOR_RGB2BGR))
            cv2.moveWindow('White Target', 40, 40)

        return white_ratio > threshold, white_ratio

    def detect_purple_upper_body(self,image, threshold=0.2):
        height, width = image.shape[:2]
        height_thre = int(height * 0.4)
        height_thre2 = int(height * 0.1)
        width_thre = int(width * 0.2)
        ...
        # 定义深紫色范围
        lower_purple = np.array([260 // 2, 50, 20])  # H:130, S>50, V>20
        upper_purple = np.array([300 // 2, 255, 180])  # H:150, S<255, V<180
        ...
        if purple_ratio > threshold:
            cv2.imshow('Purple Target', cv2.cvtColor(upper_half, cv2.COLOR_RGB2BGR))
            cv2.moveWindow('Purple Target', 80, 80)

        return purple_ratio > threshold, purple_ratio

    def detect_light_yellow_upper_body(self,image, threshold=0.2):
        height, width = image.shape[:2]
        height_thre = int(height * 0.4)
        height_thre2 = int(height * 0.1)
        width_thre = int(width * 0.2)
        ...
        # 定义浅黄色范围 (OpenCV H 是0–180，所以20°~40° -> 10~20)
        lower_yellow = np.array([24, 30, 150])
        upper_yellow = np.array([34, 80, 220])
        ...
        if yellow_ratio > threshold:
            cv2.imshow('Yellow Target', cv2.cvtColor(upper_half, cv2.COLOR_RGB2BGR))
            cv2.moveWindow('Yellow Target', 100, 100)

        return yellow_ratio > threshold, yellow_ratio

    def detect_black_upper_body(self,image, threshold=0.2):
        height, width = image.shape[:2]
        height_thre = int(height * 0.4)
        height_thre2 = int(height * 0.1)
        width_thre = int(width * 0.2)
        ...
        if black_ratio > threshold:
            cv2.imshow('Black Target', cv2.cvtColor(upper_half, cv2.COLOR_RGB2BGR))
            cv2.moveWindow('Black Target', 60, 60)
    ...
    def predict_video(self, video_file, thread_idx=0):
        ...
        #for OBS
        capture = ImageIOCapture("<video{}>".format(video_file))
        ...
        while (not framequeue.empty()):
            ...
            if frame_id > self.warmup_frame:
                ...
                if self.with_human_attr:
                    ...
                    '''added by liu for select result'''
                    # self.query_search = ['Female','Hat: False']
                    result_list = attr_res['output'] # 你的原始数据
    ...
    def visualize_video(self,
        ...
        if mot_res is not None:
            image = plot_tracking_dict(#绘制跟踪框和置信度
                image,
                num_classes,
                online_tlwhs,
                online_ids,
...
class QueryGUI(threading.Thread):
    ...
    def initialize_ui(self):
        """初始化用户界面"""
        self.root = tk.Tk()
        self.root.title("智能分析系统控制台")

        self.root.geometry("600x300+20+500")
        self.setup_styles()
        ...
        # Logo区域
        self.create_logo_frame(main_container)
    ...
    def create_logo_frame(self, parent):
        """创建Logo展示区域"""
        logo_frame = ttk.Frame(parent, relief=tk.RAISED, borderwidth=1)
        logo_frame.pack(fill=tk.X, pady=(0, 10))

        # 动态创建Logo显示
        for idx, img in enumerate(self.logo_images):
            lbl = ttk.Label(logo_frame, image=img)
            lbl.image = img  # 保持引用
            lbl.pack(side=tk.LEFT if idx == 0 else tk.RIGHT,
                     padx=20,
                     pady=10)
        ...
        for path in self.logo_paths:
            try:
                if not self.validate_image_file(path):
                    raise ValueError(f"无效的图片文件: {path}")

                img = self.process_large_image(path, target_size)
                self.logo_images.append(ImageTk.PhotoImage(img, master=self.root))

            except Exception as e:
                print(f"加载Logo错误: {str(e)}")
                self.logo_images.append(self.create_placeholder_image(target_size))

        # 确保至少有两个元素
        while len(self.logo_images) < 2:
            self.logo_images.append(self.create_placeholder_image(target_size))
    ...
    def process_large_image(self, path, target_size):
        """处理大尺寸图片的优化方法"""
        with Image.open(path) as img:
            # 渐进式加载优化
            img.draft("RGB", (target_size[0] * 2, target_size[1] * 2))
            img.thumbnail((target_size[0] * 4, target_size[1] * 4))  # 中间尺寸
    ...
    def create_input_section(self, parent):
        ...
        # 输入提示
        self.entry.insert(0, "例如：Female,Hat:False    输入ALL 显示所有")
        self.entry.config(foreground='#666666')
        self.entry.bind("<FocusIn>", self.clear_placeholder)
        self.entry.bind("<FocusOut>", self.restore_placeholder)

    def clear_placeholder(self, event):
        if self.entry.get() == "例如：Female,Hat:False    输入ALL 显示所有":
    ...
    def create_button_group(self, parent):
        """创建按钮组"""
    ...
    def update_query(self):
        """更新查询条件"""
        new_query = self.entry.get().strip()
        if new_query and new_query != "例如：Female,Hat:False    输入ALL 显示所有":
            try:
                if hasattr(self.pipeline, 'predictor'):
                    predictors = self.pipeline.predictor if isinstance(
                        self.pipeline.predictor, list) else [self.pipeline.predictor]
